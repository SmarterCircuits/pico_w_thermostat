...
class ThermostatSettings:
    ...
    def update_from_home_assistant(self, helper: HomeAssistantHelper):
        ...
        circ_time = helper.get_home_assistant_setting(
            helper.settings.circulation_time_input
        )
        if circ_time is not None:
            self.circulation_cycle_minutes = float(circ_time)

        still_time = helper.get_home_assistant_setting(
            helper.settings.circulate_after_input
        )
        if still_time is not None:
            self.air_circulation_minutes = float(still_time)

        stage_limit = helper.get_home_assistant_setting(
            helper.settings.stage_limit_input
        )
        if stage_limit is not None:
            self.stage_limit_minutes = float(stage_limit)

        stage_cooldown = helper.get_home_assistant_setting(
            helper.settings.stage_cooldown_input
        )
        ...
        temperature_offset = helper.get_home_assistant_setting(
            helper.settings.temperature_offset_input
        )
        if temperature_offset is not None:
            self.temp_offset = float(temperature_offset)

        use_vent = helper.get_home_assistant_setting(
            helper.settings.ventilation_enabled_input
        )
        if use_vent is not None:
            self.use_whole_house_fan = use_vent.lower() == "on"

        hvac_enabled = helper.get_home_assistant_setting(
            helper.settings.hvac_enabled_input
        )
