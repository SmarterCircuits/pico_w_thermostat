...
class Thermostat:
    ...
    def console_out(self, message:str):
        print(message)
        if self.ha_settings.enabled is False:
            return
        try:
            self.ha_helper.send_to_home_assistant(self.ha_settings.console_output, message)
        except:
            print("failed to send to HA")
    ...
    def start_circulating(self, now_time:tuple):
        self.console_out(f"start_circulating {now_time}")
        self.fan.off()
